...
export function InstructorPage() {
  ...
  const foundName =
    Object.keys(instructorsDb).find((name) => {
      ...
      <main className="flex-grow pt-28 pb-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <Link
              to="/"
              className="inline-flex items-center gap-2 text-xs text-text-secondary hover:text-gold-hover transition"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-4 space-y-6">
              <GoldCard className="p-6 text-center border-gold-border/15">
                ...
                <p className="text-xs text-gold-primary mt-2 font-medium leading-relaxed">
                  {instructor.title}
                </p>
                ...
                <div className="flex justify-center gap-4 mt-6 pt-6 border-t border-border-subtle">
                  {instructor.socials.linkedin && (
                    <a
                      ...
                      className="p-2 rounded-full border border-gold-border/20 text-text-secondary hover:text-gold-hover hover:border-gold-primary transition"
                      aria-label="LinkedIn"
                    >
              ...
              <GoldCard className="p-6 border-gold-border/15">
                ...
                <div className="flex flex-wrap gap-2">
                  {instructor.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="bg-bg-elevated border border-gold-border/20 text-gold-primary text-xs px-3 py-1.5 rounded-xl font-medium"
                    >
                      {skill}
                    </span>
                  ))}
            ...
            <div className="lg:col-span-8 space-y-12">
              ...
              <section className="space-y-6">
                <div className="space-y-3">
                  <h2 className="text-3xl font-display font-bold text-text-primary border-r-4 border-gold-primary pr-3 leading-none">
              ...
              <section className="space-y-6">
                ...
                {instructor.coursesTaught.length === 0 ? (
                  <p className="text-text-secondary text-sm">
                    لا توجد دورات مسجلة حالياً لهذا المدرب.
                  </p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {instructor.coursesTaught.map((c) => (
                      <GoldCard
                        key={c.id}
                        className="group relative overflow-hidden hover:-translate-y-1.5 transition flex flex-col justify-between h-full"
                      >
                        <div>
                          ...
                          <div className="p-5">
                            <h3 className="text-text-primary font-bold text-base leading-snug min-h-[3rem] line-clamp-2">
                              {c.title}
                            </h3>
                          </div>
                        </div>
                        <div className="p-5 pt-0 flex items-center justify-between border-t border-border-subtle mt-4">
                          <span className="text-gold-primary font-serif text-xl font-bold">
                            {toArabicDigits(c.price)}
                          </span>
                          <Link
                            to={`/course/${c.id}`}
                            className="text-xs text-gold-primary underline hover:text-gold-hover font-semibold"
                          >
                            عرض تفاصيل الدورة ←
                          </Link>
                        </div>
