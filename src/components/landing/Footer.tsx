      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            <button className="rounded-lg border border-gold-border bg-bg-card px-4 py-2 text-xs text-text-primary hover:border-gold-primary hover:scale-[1.03] active:scale-[0.98] transition-all duration-300 ease-supportive cursor-pointer">Google Play</button>
            <button className="rounded-lg border border-gold-border bg-bg-card px-4 py-2 text-xs text-text-primary hover:border-gold-primary hover:scale-[1.03] active:scale-[0.98] transition-all duration-300 ease-supportive cursor-pointer">App Store</button>
          </div>
          <div>
            <p className="text-xs text-text-secondary mb-2">بوابات الدفع الآمنة</p>
            <div className="flex flex-wrap gap-2">
              {["Visa", "Mastercard", "Apple Pay", "Mada"].map((p) => (
                <span key={p} className="rounded border border-gold-border/75 bg-bg-card px-2 py-1 text-[10px] text-text-secondary">{p}</span>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-text-secondary">
            {categories.slice(0, 5).map((category) => category.name).map((l) => (
              <li key={l}><a href="#" className="hover:text-gold-primary transition-colors duration-300">{l}</a></li>
            ))}
          </ul>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <a key={label} href="#" aria-label={label} className="flex h-10 w-10 items-center justify-center rounded-full border border-gold-border text-text-secondary hover:text-gold-primary hover:border-gold-primary hover:scale-[1.05] transition-all duration-300 ease-supportive">
                <Icon className="h-4 w-4" />
              </a>
            ))}
          </div>
          <p className="mt-6 text-xs text-text-muted">استكشف {toArabicDigits(landingStats.courses)} دورة و{toArabicDigits(landingStats.consultancies)} استشارة في ماستري اكاديمي</p>
        </div>
      </div>
